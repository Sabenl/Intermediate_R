...
# Your code here
files_list <- ________(______, pattern = ________, full.names = TRUE, recursive = T)
files_list
...
# Your code here
data_list <- ____(files_list, ~ ______(______))
data_list
...
# Your code here
data_list_named <-  data_list %>% set_names(______(basename(______), "\\.csv"))
____(data_list_named, ~head(____, 5))
...
# Your code here
combined_df <- ______(data_list_named)
combined_df
...
combined_df %>%
  group_by(______) %>% 
  ______(n_missing = ______(is.na(Value)))
...
combined_df %>%
  ______(Date) %>%
  summarise(Mean_Value = ______(Value))
```
...
imputed_df <- combined_df %>%
  group_by(Date) %>% 
  mutate(Value = case_when(______(Value) ~ median(Value, ______ = T),
                           TRUE ~ ______)) %>% 
...
wide_df <- imputed_df %>%
  ________(Month) %>% 
  pivot_wider(________ = Category, ________ = Value)
wide_df
...
mdn_df <- wide_df %>%
  ________() %>% 
  group_by(Date) %>% 
  mutate(Mdn_value = ________(c_across(A:C), ________ = T))
mdn_df
...
filtered_june <- imputed_df %>%
  filter(Month == 'June') %>% 
  ________(Category) %>% 
  ________(-c(which.min(Value), ________(Value)))
filtered_df
...
recode_df <- top_10_rows %>%
  ________(Category = ________(Value >= 51 ~ 'A',
...
processed_df <- top_10_rows %>% 
  ________(names_from = 'Category', values_from = 'Value') 
...
processed_df <- processed_df %>% 
  bind_cols(., map2_dfc(functions_list, c('log2', '(x-mean)/sd', '(x-median)/range'), function(func, abbr) {
                map_dfc(processed_df %>% select(A:C), ~ func(.)) %>% 
                  setNames(paste0(colnames(processed_df %>% ______(A:C)), "_", abbr))
}))

processed_df <- processed_df %>%
  ________(vars(A:C), ~paste0(., '_raw'))
...
# Your code here
ggplot(mdn_df, aes(x = ______, y = Mdn_value, color = Month)) +
  ________() +
...
# Your code here
processed_df %>% 
  ________(cols = -c(Date, Month), names_to = 'Name', values_to = 'Value') %>% 
  separate(Name, into = c('Category', 'Type'), sep = ____, remove = F) %>% 
  ggplot(., aes(x = Category, y = Value, fill = Category)) +
  geom_boxplot() +
  ________(~Type, scales = 'free_y') +
  labs(x = "",
       y = "Value") +
  theme(legend.position = 'none')
