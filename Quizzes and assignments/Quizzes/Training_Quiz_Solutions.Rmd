...
### Attach pckgs
- We will need functions from tidyverse for this exercise to work.
...
files_list <- list.files("./data/Timeseries_data", pattern = "csv$", full.names = TRUE, recursive = T)
files_list
...
```{r}
# Use set_names(), str_remove() and basename() functions to assign meaningful names for each data frame in the list. Use corresponding file names stored in the "files_list" object to achieve this while omitting CSV extension.
...
processed_df <- processed_df %>% 
  bind_cols(., map2_dfc(functions_list, c('log', '(x-mean)/sd', '(x-median)/range'), function(func, abbr) {
